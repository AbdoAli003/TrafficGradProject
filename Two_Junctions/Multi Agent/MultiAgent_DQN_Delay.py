...
for episode in range(episodes):
  ...
  for step in range(TOTAL_STEPS):
    ...
    if step % 1 == 0:
       update_Q_table(compress_state(state), action, reward/10.0, compress_state(new_state))
...
data.to_csv("combine graphs/MultiAgent_DQN_Delay_results.csv", index=False)