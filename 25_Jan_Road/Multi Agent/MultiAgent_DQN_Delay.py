...
for episode in range(episodes):
  ...
  for step in range(TOTAL_STEPS):
    ...
    if step % 1 == 0:
        for tls in tls_ids:
          update_Q_table(tls,compress_state(state),actions[tls],reward/100.0,compress_state(new_state))
    ...
    if step % 100 == 0:
        print(f"\nepsilon : {EPSILON} - MultiAgent_DQN_Delay.py:363")
...
print("\nOnline Training completed. - MultiAgent_DQN_Delay.py:380")
print("DQN Models Summary: - MultiAgent_DQN_Delay.py:381")
for tls in tls_ids:
    print("\nAgent: - MultiAgent_DQN_Delay.py:383",tls)
