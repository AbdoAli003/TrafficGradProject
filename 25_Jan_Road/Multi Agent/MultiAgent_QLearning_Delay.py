...
for episode in range(episodes):
  ...
  for step in range(TOTAL_STEPS):
    ...
    if step % 100 == 0:
        ...
        print(f"\nstate:{state} - MultiAgent_QLearning_Delay.py:320")
        print(f"epsilon :{EPSILON} - MultiAgent_QLearning_Delay.py:321")
        print(f"Step {step}, Current_State: {state}, Action: {actions}, New_State: {new_state}, Reward: {reward:.2f}, Cumulative Reward: {cumulative_reward:.2f} - MultiAgent_QLearning_Delay.py:322")
        for tls in tls_ids:
           print(f"TLS {tls} Qtable size: {len(Q_tables[tls])} - MultiAgent_QLearning_Delay.py:324")
 # -------------------------
...
if selected_demand == high:
    ttc_data.to_csv("combine graphs/high_demand_MultiAgent_QLearning_Delay_TTC_results.csv", index=False)
elif selected_demand == medium:
    ttc_data.to_csv("combine graphs/medium_demand_MultiAgent_QLearning_Delay_TTC_results.csv", index=False)
elif selected_demand == low:
    ttc_data.to_csv("combine graphs/low_demand_MultiAgent_QLearning_Delay_TTC_results.csv", index=False)