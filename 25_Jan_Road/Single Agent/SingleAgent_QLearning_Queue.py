...
ALPHA = 0.2          # Learning rate (α) between[0, 1]    #If α = 1, you fully replace the old Q-value with the newly computed estimate.
                                                            #If α = 0, you ignore the new estimate and never update the Q-value.
GAMMA = 0.95           # Discount factor (γ) between[0, 1]  #If γ = 0, the agent only cares about the reward at the current step (no future rewards).
...
print("Edge groups: - SingleAgent_QLearning_Queue.py:111", edge_detectors)
# -------------------------
...
def inject_breakdown(lane_id,edge_id, duration=100):
    ...
    try:
      if stopping_car == None :
        ...
        print(f"Breakdown injected on vehicle {veh_id} at edge {edge_id} and lane {lane_id} for {duration} steps. - SingleAgent_QLearning_Queue.py:147")
        stopping_car = veh_id

    except traci.TraCIException:
        print("not found - SingleAgent_QLearning_Queue.py:151")
...
print("\n=== Starting Fully Online Continuous Learning === - SingleAgent_QLearning_Queue.py:273")
episodes = 1
for episode in range(episodes):
  ...
  print(f"start of episode {episode} - SingleAgent_QLearning_Queue.py:285")
  action_step = 0
  last_action = []
  last_action_state = []
  for step in range(TOTAL_STEPS):
    ...
    if step % 1 == 0:
      update_Q_table(compress_state(last_action_state),last_action, reward/(NUM_detectors*8.0),compress_state(new_state))
    ...
    if step % 100 == 0:
        print(f"\nepsilon :{EPSILON} - SingleAgent_QLearning_Queue.py:314")
        print(f"Step {step}, Current_State: {state}, Action: {action}, New_State: {new_state}, Reward: {reward:.2f}, Cumulative Reward: {cumulative_queue_reward:.2f} - SingleAgent_QLearning_Queue.py:315")
        ...
        print(f"Current Qtable length {len(Q_table)}: - SingleAgent_QLearning_Queue.py:323")
  # -------------------------
...
print("\nOnline Training completed. Final Qtable size: - SingleAgent_QLearning_Queue.py:330", len(Q_table))
for st, actions in Q_table.items():
    print("State: - SingleAgent_QLearning_Queue.py:332", st, "-> Q-values:", actions)
...
if selected_demand == high:
    ttc_data.to_csv("combine graphs/high_demand_SingleAgent_QLearning_Queue_TTC_results.csv", index=False)
elif selected_demand == medium:
    ttc_data.to_csv("combine graphs/medium_demand_SingleAgent_QLearning_Queue_TTC_results.csv", index=False)
elif selected_demand == low:
    ttc_data.to_csv("combine graphs/low_demand_SingleAgent_QLearning_Queue_TTC_results.csv", index=False)