...
from traci.constants import CMD_EXECUTEMOVE  # Visualization
...
TOTAL_STEPS = 10000    # Total simulation steps
...
def get_reward():
    """
    Reward = Negative Total Waiting Time (Delay)
    Minimizing delay directly.
    """
    total_delay = 0.0
...
for episode in range(episodes):
  ...
  for step in range(TOTAL_STEPS):
    ...
    updated_q_vals = Q_table[compress_state(state)]
    if step % 20 == 0 and step > 0:
      if EPSILON > EPSILON_MIN:
        EPSILON *= EPSILON_DECAY
    """
    state = get_state()
    state_discretized = tuple(discretize_queue(q) for q in state[:-1]) + (state[-1],)  # keep phase discrete
    action = get_action_from_policy(state_discretized)
    apply_action(action)
    traci.simulationStep()
    new_state = get_state()
    new_state_discretized = tuple(discretize_queue(q) for q in new_state[:-1]) + (new_state[-1],) 
    reward = get_reward()
    cumulative_reward += reward
    
    update_Q_table(state_discretized, action, reward, new_state_discretized)
    
    # Print Q-values for the old_state right after update
    updated_q_vals = Q_table[state_discretized]
    """
    # Record data every step
    if step % 100 == 0:
      print(f"\nstate:{state} - SingleAgent_QLearning_Delay.py:329")
      print(f"epsilon :{EPSILON} - SingleAgent_QLearning_Delay.py:330")
      print(f"Step {step}, Current_State: {state}, Action: {action}, New_State: {new_state}, Reward: {reward:.2f}, Cumulative Reward: {cumulative_reward:.2f}, Qvalues(current_state): {updated_q_vals} - SingleAgent_QLearning_Delay.py:331")
...
print("\nOnline Training completed. Final Qtable size: - SingleAgent_QLearning_Delay.py:343", len(Q_table))
for st, actions in Q_table.items():
    print("State: - SingleAgent_QLearning_Delay.py:345", st, "-> Q-values:", actions)
