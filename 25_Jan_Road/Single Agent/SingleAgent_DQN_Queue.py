...
print("Edge groups: - SingleAgent_DQN_Queue.py:115", edge_detectors)
# -------------------------
...
def inject_breakdown(lane_id,edge_id, duration=100):
    ...
    try:
      if stopping_car == None :
        ...
        print(f"Breakdown injected on vehicle {veh_id} at edge {edge_id} and lane {lane_id} for {duration} steps. - SingleAgent_DQN_Queue.py:151")
        stopping_car = veh_id

    except traci.TraCIException:
        print("not found - SingleAgent_DQN_Queue.py:155")
...
print("\n=== Starting Fully Online Continuous Learning (DQN) === - SingleAgent_DQN_Queue.py:344")
episodes = 1
for episode in range(episodes):
  ...
  for step in range(TOTAL_STEPS):
    ...
    if step % 100 == 0:
        print(f"\nepsilon : {EPSILON} - SingleAgent_DQN_Queue.py:391")
        print(f"Step {step}, Current_State: {state}, Action: {action}, New_State: {new_state}, Reward: {reward:.2f}, Cumulative Reward: {cumulative_reward:.2f} - SingleAgent_DQN_Queue.py:392")
...
print("\nOnline Training completed. - SingleAgent_DQN_Queue.py:402")
print("DQN Model Summary: - SingleAgent_DQN_Queue.py:403")
dqn_model.summary()
